...
def run_vqe(H):
    ...
    params = np.random.uniform(low=-np.pi/2, high=np.pi/2, size = (num_param_sets, 1))
    circuit = variational_ansatz
    cost_fn = qml.ExpvalCost(circuit, H, dev)
    opt = qml.GradientDescentOptimizer(stepsize=0.12)
    max_iterations = 500
    conv_tol = 1e-07
    for n in range(max_iterations):
        ...
        if n % 20 == 0:
            print('Iteration = {:}, Energy = {:.7f} Ha'.format(n, energy))
        if conv <= conv_tol:
            break
